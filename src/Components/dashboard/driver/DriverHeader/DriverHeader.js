      ...
      .then((data) => {
        if (data.success && data.data) {
          setDriverInfo({
            ...
            photoUrl:
              data.data.photo && data.data.photo.trim() !== ""
                ? data.data.photo
                : null,
          });
            ...
            fetch(
              ...
              .then((statsData) => {
                if (statsData.success && statsData.stats) {
                  ...
                  console.log("✅ Driver stats loaded:", statsData.stats);
                }
              })
              .catch((err) => {
    ...
    const fetchRevenue = async () => {
      ...
      try {
        ...
        const resp = await fetch(
          `http://localhost/RoutePro-backend(02)/public/driver/revenue?email=${encodeURIComponent(
            userEmail
          )}`,
          {
            method: "GET",
        ...
        const value =
          (data &&
            data.data &&
            (data.data.revenue ?? data.data.totalRevenue)) ??
          null;
        setRevenue(value);
      } catch (err) {
    ...
    <div className="driver-header">
      <div className="photo-wrapper">
        <img
          src={
            photoPreview ||
            (driverInfo.photoUrl && driverInfo.photoUrl.trim() !== ""
              ? `http://localhost${driverInfo.photoUrl}?t=${Date.now()}`
      ...
      <div className="driver-info">
        ...
        <p>
          <strong>License No:</strong> {driverInfo.license_no || "Not provided"}
        </p>
        <p>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      // Update profile data
                      const response = await fetch(
                        "http://localhost/RoutePro-backend(02)/public/driver/profile",
                        {
                          method: "PUT",
                          headers: { "Content-Type": "application/json" },
                      ...
                      if (photoFile) {
                        ...
                        const photoResponse = await fetch(
                          "http://localhost/RoutePro-backend(02)/public/driver/photo",
                          {
                            method: "POST",
                            body: form,
                          }
                        ...
                        if (photoResponse.ok) {
                          ...
                          console.warn(
                            "⚠️ Photo upload failed, but profile was updated"
                          );
                        }
                      }
                      ...
                      if (refreshed.ok) {
                        ...
                        if (refreshedData.success && refreshedData.data) {
                          setDriverInfo({
                            ...
                            vehicle_type:
                              refreshedData.data.vehicle_type ||
                              payload.vehicle_type,
                            experience:
                              refreshedData.data.experience ||
                              payload.experience,
                            location:
                              refreshedData.data.location || payload.location,
                            photoUrl:
                              refreshedData.data.photo &&
                              refreshedData.data.photo.trim() !== ""
                                ? refreshedData.data.photo
                                : null,
                          });
