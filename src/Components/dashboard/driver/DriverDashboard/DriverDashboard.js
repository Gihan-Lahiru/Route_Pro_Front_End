...
const DriverDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (userId) {
      ...
      fetch(
        `http://localhost/RoutePro-backend(02)/public/api/auth/user-info.php?user_id=${userId}`
      )
        .then((res) => {
        ...
        .then((userData) => {
          ...
          if (userEmail) {
            ...
            return fetch(
              `http://localhost/RoutePro-backend(02)/public/driver/profile?email=${encodeURIComponent(
                userEmail
              )}`,
              {
                method: "GET",
        ...
        .then((data) => {
          if (data) {
            ...
            if (
              ...
              console.log(
                "✅ Got driver name from driver profile API:",
                driverName
              );
            } else {
              console.log(
                "❌ No valid name found in driver profile API response"
              );
      ...
      console.log(
        "🚀 No user ID found, trying driver profile API with email:",
        userEmail
      );
  ...
  useEffect(() => {
    ...
    if (userId) {
      console.log("🔍 Fetching driver stats for userId:", userId);

      fetch(
        `http://localhost/RoutePro-backend(02)/public/api/reviews/reviews.php?driver_id=${userId}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.success && data.stats) {
            setDriverStats({
              average_rating: parseFloat(data.stats.average_rating) || 0,
              total_reviews: data.stats.total_reviews || 0,
            });
            console.log("✅ Driver stats loaded:", data.stats);
          }
        })
        .catch((err) => {
  ...
  useEffect(() => {
    ...
    const userId = localStorage.getItem("userId");
    if (userId) {
      console.log("🔍 Fetching trip counts for driver:", userId);

      fetch(
        `http://localhost/RoutePro-backend(02)/public/api/trips/driver-trips.php?driver_id=${userId}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.success && data.trips) {
            // Count trips by status
            const confirmedCount = data.trips.filter(
              (trip) => trip.trip_status === "confirmed"
            ).length;
            const completedCount = data.trips.filter(
            ...
            console.log("✅ Trip counts loaded:", {
              confirmed: confirmedCount,
              completed: completedCount,
            });
          }
  ...
  const checkIfCanGoAvailable = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost/RoutePro-backend(02)/public/driver/rating-check/?userId=${userId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateStatusInBackend = async (newStatus) => {
    try {
      ...
      const response = await fetch(
        "http://localhost/RoutePro-backend(02)/public/driver/status",
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="dashboard">
      ...
      <div className="summary-row">
        ...
        <div className="summary-card">
          ...
          <p>
            ⭐ {driverStats.average_rating.toFixed(1)} (Based on{" "}
            {driverStats.total_reviews} reviews)
          </p>
        </div>
