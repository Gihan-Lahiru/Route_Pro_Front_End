...
const TripDetails = () => {
  ...
  const handleStartTrip = async (tripId) => {
    try {
      ...
      console.log(
        `🗺️ Guide starting trip ${tripId} at ${new Date().toLocaleTimeString()}`
      );
      ...
      if (result.success) {
        ...
        console.log(`✅ Trip ${tripId} guide tour started`);
  ...
  const handleCompleteTrip = async (tripId) => {
    try {
      ...
      console.log(
        `🏁 Guide completing trip ${tripId} at ${completedAt.toLocaleTimeString()}`
      );
      ...
      if (result.success) {
        ...
        if (result.trip_duration) {
          ...
          let durationMessage = `Guide tour completed successfully!\n\n⏱️ Tour Duration: ${duration} minutes`;

          if (duration <= 5) {
            durationMessage += `\n🎯 Quick tour! Completed in ${duration} minutes (≤5 min target)`;
          } else if (duration <= 30) {
            durationMessage += `\n🗺️ Standard tour duration (${duration} minutes)`;
          } else {
            durationMessage += `\n📍 Extended tour (${duration} minutes)`;
          }

          alert(durationMessage);
          console.log(`📊 Guide Trip ${tripId} Duration Analysis:`, {
            duration: duration,
            isQuick: duration <= 5,
            isStandard: duration <= 30,
