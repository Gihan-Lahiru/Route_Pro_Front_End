    ...
    const fetchRevenue = async () => {
      ...
      try {
        const resp = await fetch(
          `http://localhost/RoutePro-backend(02)/public/guide/revenue?email=${encodeURIComponent(
            userEmail
          )}`,
          {
            method: "GET",
    ...
    <div className="driver-header">
      <div className="photo-wrapper">
        <img
          src={
            photoPreview ||
            (guiderInfo.photoUrl && guiderInfo.photoUrl.trim() !== ""
              ? `http://localhost${guiderInfo.photoUrl}?t=${Date.now()}`
      ...
      <div className="guide-info">
        ...
        <p>
          ⭐ {guideStats.average_rating.toFixed(1)} ({guideStats.total_reviews}{" "}
          reviews)
        </p>
        <p>
                ...
                onClick={async () => {
                  try {
                    ...
                    const response = await fetch(
                      "http://localhost/RoutePro-backend(02)/public/guide/profile",
                      {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                    ...
                    if (photoFile) {
                      ...
                      try {
                        await fetch(
                          "http://localhost/RoutePro-backend(02)/public/guide/photo",
                          {
                            method: "POST",
                            body: form,
                          }
                    ...
                    if (refreshResponse.ok) {
                      ...
                      if (refreshedData.success && refreshedData.data) {
                        const updatedInfo = {
                          ...
                          languages:
                            refreshedData.data.languages || payload.languages,
                          experience:
                            refreshedData.data.experience || payload.experience,
                          location:
                    ...
                    alert(
                      `Could not save profile: ${err.message}. Please try again.`
                    );
                  } finally {
                    setIsSaving(false);
                  }
